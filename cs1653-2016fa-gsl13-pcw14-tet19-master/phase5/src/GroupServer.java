...
public class GroupServer extends Server {
	...
	public Hashtable<String, UserStatus> timeoutTable = new Hashtable<String, UserStatus>();
	public static ArrayList<String> commonPasswords = new ArrayList<String>();
    ...
    FileInputStream gis = null;
		...
		try
		{
			...
			// Create list of failed authentications from users
			for (String username : userList.list.keySet())
				timeoutTable.put(username, new UserStatus(username));
		...
		catch(FileNotFoundException e)
		{
			...
			int validPassword = checkPassword(password);
			while(validPassword!=0){
				...
				{
					System.out.print("Password Contains a commmon word, BE CREATIVE: ");
					password = console.next();
					validPassword = checkPassword(password);
				}
			}
			...
			timeoutTable.put(username, userstatus);
	...
	public void ReadFile()
  {
		...
		try {
			...
			in = new Scanner(commonpasswordsfile);
			String str;
			...
			System.out.println("common passwords length " + commonPasswords.size());
		}
		catch (Exception e) {
		//	e.printStackTrace();
		}
