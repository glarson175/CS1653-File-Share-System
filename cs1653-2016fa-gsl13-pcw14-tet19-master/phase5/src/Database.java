...
public class Database implements java.io.Serializable {
		...
		//mutator
		public synchronized boolean updateItem(String username, String newPass){
			if(checkItemExists(username)){
				try{
					byte[] newSalt = crypto.generateSalt(256);
					//generate pass bytes
					byte[] newPassBytes = newPass.getBytes(StandardCharsets.UTF_8);
					//concatenate together for hashing
					ByteArrayOutputStream outputStream = new ByteArrayOutputStream( );
					outputStream.write( newSalt );
					outputStream.write( newPassBytes );
					byte[] item = outputStream.toByteArray( );
					//Hash it
					MessageDigest digest = MessageDigest.getInstance("SHA-256");
					byte[] newHashedPass = digest.digest(item);
					//get the item from the list and update it
					list.get(username).update(newHashedPass, newSalt);
					System.out.println("Successfully updated user with new password!");
					return true;
				}
				catch( Exception e){
					System.out.println("Database Error: " + e.toString());
