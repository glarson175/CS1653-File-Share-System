...
//	private static ArrayList <String> commonPasswords = new ArrayList();
			...
			do
			{
				...
				//STEP 3 -
					...
					try{
						if(authenticate1){
							...
							user = (String)message.getObjContents().get(1);
							System.out.println("Username: " + user);
							String pass =  (String)message.getObjContents().get(2);

							// Check to see if user is on timeoutTable
							UserStatus userStatus = my_gs.timeoutTable.get(user);

							if (userStatus != null && userStatus.isLocked()){
								System.out.println("User's account is currently on timeout for too many failed username/password attempts!");
								response = new Envelope("FAIL-USERLOCKED");
							}
							else if (my_gs.database.checkItemExists(user) && my_gs.database.checkItem(user, pass)){
								...
								// reset user fails
								userStatus = my_gs.timeoutTable.get(user);
								userStatus.resetFails();

								// make deep copy of fail list
								Hashtable<String, UserStatus> copyTable = new Hashtable<String, UserStatus>();
								for (String name : my_gs.timeoutTable.keySet()){
									UserStatus copy = UserStatus.deepCopy(my_gs.timeoutTable.get(name));
									copyTable.put(name, copy);
								}
								my_gs.timeoutTable = copyTable;
								userStatus = my_gs.timeoutTable.get(user);
								System.out.println("User is at " + userStatus.getFails() +  " fails");

								//Extract out the hash of the file server's public key
								byte[] fsKeyHash = (byte[])message.getObjContents().get(3);
								...
								response = new Envelope("FAIL-USER/PASSWORD");
								message.addObject(null);
								...
								if (userStatus != null){
									...
									userStatus = my_gs.timeoutTable.get(user);
									System.out.println("User is at " + userStatus.getFails() +  " fails");
									System.out.println("User is locked : "+ userStatus.isLocked());
								}
								else {
							...
							response = new Envelope("FAIL-AUTH1");
						}
					} catch(Exception e){
						e.printStackTrace();
				...
				else if(message.getMessage().equals("CUSER")) //Client wants to create a user
				{
					...
					try{
						...
						int passcheck = my_gs.checkPassword(password);
						if (passcheck == 1)
						{
							System.out.println("Something failed when trying to create user.");
							response = new Envelope("FAIL-BADPASSWORD_LENGTH"); //something didn't work
							System.out.println("Sending back FAIL response. bad password");
						}
						else if (passcheck == 2){
							System.out.println("Something failed when trying to create user.");
							response = new Envelope("FAIL-BADPASSWORD_CONTAINSCOMMONWORD"); //something didn't work
							System.out.println("Sending back FAIL response. bad password");
						}
						else{
							...
							if(createUser(username, yourToken))
							{
								...
								if(my_gs.database.addItem(username, password)){
									...
									// add user to UserStatus list
									UserStatus userStatus = new UserStatus(username);
 									my_gs.timeoutTable.put(username, userStatus);
								}
							}
							else{
				...
				//Phase 5: change pw
					...
					response = null;
					System.out.println(user + " is trying to change password.");
					try{
						String oldPass = (String)message.getObjContents().get(1);
						String newPass = (String)message.getObjContents().get(2);
						Token token = (Token)message.getObjContents().get(3);
						int checkpass = my_gs.checkPassword(newPass);
						if(checkpass == 1){
							System.out.println("Password too short. User did NOT successfully change password.");
							response = new Envelope("FAIL-BADPASSWORD_LENGTH");
						}
						else if(checkpass == 2){
							System.out.println("Password contains common password. User did NOT successfully change password.");
							response = new Envelope("FAIL-BADPASSWORD_CONTAINSCOMMONWORD");;
						}
						else{
							if (changePassword(oldPass, newPass, token)) {
								response = new Envelope("OK"); //Successdddddd
							}
							else {
								System.out.println("User did NOT successfully change password.");
								response = new Envelope("FAIL-CHANGEPASS"); //something didn't work
							}
						}
					}
					catch(Exception e){
  ...
  {
		File commonpasswordsfile = new File("1Kmostcommon.txt");
		//Scanner readFile = new Scanner(dictFile);
        Scanner in = new Scanner(commonpasswordsfile);
				String str;
        while(in.hasNextLine())
        {
					str = in.nextLine();
           commonPasswords.add(str);
					//System.out.println(str + " has been added to list");
        }
   }
	...
	public static int checkPassword(String pass){
		Envelope response;
			if(pass.length()<9){
				...
				{
					if(pass.toLowerCase().contains(commonPasswords.get(i).toLowerCase()))
					{
						return 2;
					}
				}
	...
	boolean changePassword(String oldPass, String newPass, UserToken token){
		String user = token.getSubject();
		if(!my_gs.database.checkItemExists(user)){
			//user does not exist
			System.out.println("User does not exist.");
			return false;
		}
		else{
			if(my_gs.database.checkItem(user,oldPass)){
				System.out.println("Current Password was correct! Changing password now.");
				if(my_gs.database.updateItem(user, newPass)){
					System.out.println("Successfully changed user's password in database.");
					return true;
				}
				else{
					System.out.println("Something went wrong when changing entry in database.");
					return false;
				}
			}
			else{
				System.out.println("Current Password was NOT correct.");
				return false;
			}
		}
	}
