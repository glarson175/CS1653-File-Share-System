...
public class Crypto{
		...
		public byte[] hashSHA_256(byte[] b){
			try{
				MessageDigest md = MessageDigest.getInstance("SHA-256");
				md.update(b);
				byte[] digest = md.digest();
	...
	public byte[] hmac (byte[] message, Key key) {
		byte[] messageHmac = null;
		try{
			Mac hmac = Mac.getInstance("HmacSHA256", "BC");
			hmac.init(key);
			messageHmac = hmac.doFinal(message);
		}
		catch (Exception e) {
			System.out.println("problem with hmac");
			e.printStackTrace();
		}
		return messageHmac;
	}

	public boolean verifyHmac(byte[] hmac, byte[] message, Key key) {
		byte[] hmac_current = hmac(message, key);
		return MessageDigest.isEqual(hmac, hmac_current);
	}
