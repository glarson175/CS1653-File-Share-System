...
public class ClientGUI{
  ...
  private static final String[] options = {"","View My Info", "Upload File", "Download File",
                              ...
                              "List Members of Group", "Create User", "Delete User", "Change Password"};
  public ClientGUI(){
    keylist = readKeyList();
...
// Menu item handlers ----------------------------------------
          ...
          String group = groupnameField.getText();
                ...
                System.out.println("FileKeyPair being used for encryption: " + kp.toString() );
								...
								if(f.exists()){
									...
									if(f_client.upload(source, dest, group, token, keyIndex, kp)){
    ...
    submit.addActionListener(new ActionListener()
      {
        ...
        //  String filename = sourceField.getText();
          if(!list.isSelectionEmpty()){
            String[] names = ((String)(list.getSelectedValue())).split(": ");
  ...
  private void changePassword(){
    actionPanel.removeAll();
    JLabel prompt = new JLabel("You Selected to Change Your Password");
    addComponent(0,0,1,1,actionPanel,prompt);
    JLabel currentPassLabel = new JLabel("Current Password:");
    addComponent(0,1,1,1,actionPanel, currentPassLabel);
    JTextField currentPassField = new JTextField(20);
    addComponent(1,1,1,1,actionPanel, currentPassField );
    JLabel newPassLabel = new JLabel("New Password:");
    addComponent(0,2,1,1,actionPanel, newPassLabel);
    JTextField newPassField = new JTextField(20);
    addComponent(1,2,1,1,actionPanel, newPassField);
